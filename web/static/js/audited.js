...
export function formatAudited(timestamp) {
  ...
  if (Number.isNaN(then.getTime())) return "never audited";
  ...
  if (elapsed < 60 * 1000) return "audited just now";

  for (const [unit, ms] of UNITS) {
    const value = Math.floor(elapsed / ms);
    if (value >= 1) return `audited ${relative.format(-value, unit)}`;
  }
  return "audited just now";
}
