...
Numbers `12`–`20` and `24` are accepted core work, `21`–`23` and `25` are
claimed by spikes, and `26`–`49` remain reserved (see `00-overview.md` for
the shared spec/spike number space).
Nothing in specs `00`–`24` may depend on anything defined here: the
