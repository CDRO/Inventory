...
Numbers `12`–`20`, `24` and `26`–`28` are accepted core work, `21`–`23` and
`25` are claimed by spikes, and `29`–`49` remain reserved (see
`00-overview.md` for the shared spec/spike number space).
Nothing in specs `00`–`28` may depend on anything defined here: the
